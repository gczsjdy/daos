	   ...
	   struct ds_rsvc **svcp)
{
	...
	if (rc != ABT_SUCCESS) {
		D_ERROR("%s: failed to create mutex: %s\n", svc->s_name,
			d_errstr(rc));
	...
	if (rc != ABT_SUCCESS) {
		D_ERROR("%s: failed to create state_cv: %s\n", svc->s_name,
			d_errstr(rc));
		rc = dss_abterr2der(rc);
		goto err_mutex;
	}
	...
	if (rc != ABT_SUCCESS) {
		D_ERROR("%s: failed to create leader_ref_cv: %s\n", svc->s_name,
			d_errstr(rc));
		rc = dss_abterr2der(rc);
		goto err_state_cv;
	}
	...
	if (rsvc_class(class)->sc_map_dist != NULL) {
		...
		if (rc != ABT_SUCCESS) {
			D_ERROR("%s: failed to create map_dist_cv: %s\n",
				svc->s_name, d_errstr(rc));
			rc = dss_abterr2der(rc);
...
init_map_distd(struct ds_rsvc *svc)
{
	...
	rc = dss_ult_create(map_distd, svc, DSS_ULT_MISC, DSS_TGT_SELF, 0,
			    &svc->s_map_distd);
	if (rc != 0) {
		D_ERROR("%s: failed to start map_distd: %s\n", svc->s_name,
			d_errstr(rc));
		put_leader(svc);
		ds_rsvc_put(svc);
	}
...
rsvc_step_up_cb(struct rdb *db, uint64_t term, void *arg)
{
	...
	if (rc == DER_UNINIT) {
		...
		D_ERROR("%s: failed to step up as leader "DF_U64": %s\n",
			svc->s_name, term, d_errstr(rc));
		if (rsvc_class(svc->s_class)->sc_map_dist != NULL)
			fini_map_distd(svc);
		goto out_mutex;
	}
...
out_mutex:
	ABT_mutex_unlock(svc->s_mutex);
	D_DEBUG(DB_MD, "%s: bootstrapped: %s\n", svc->s_name, d_errstr(rc));
	return rc;
}
...
rsvc_stop_cb(struct rdb *db, int err, void *arg)
{
	...
	rc = dss_ult_create(rsvc_stopper, svc, DSS_ULT_MISC, DSS_TGT_SELF,
			    0, NULL);
	if (rc != 0) {
		D_ERROR("%s: failed to create service stopper: %s\n",
			svc->s_name, d_errstr(rc));
		ds_rsvc_put(svc);
	}
}
...
ds_rsvc_stop_all(enum ds_rsvc_class_id class)
{
	...
	if (rc != 0)
		D_ERROR("failed to stop all replicated services: %s\n",
			d_errstr(rc));
	return rc;
}
		   ...
		   bool bootstrap, size_t size)
{
	...
	rc = out->sao_rc;
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to start%s replicas: %s\n",
			DP_UUID(dbid), create ? "/create" : "", d_errstr(rc));
		  ...
		  bool destroy)
{
	...
	rc = out->soo_rc;
	if (rc != 0) {
		D_ERROR("failed to stop%s replicas: %s\n",
			destroy ? "/destroy" : "", d_errstr(rc));
		rc = -DER_IO;
	}
