		   ...
		   daos_handle_t coh, void *priv, struct btr_context **tcxp)
{
 ...
 failed:
	D_DEBUG(DB_TRACE, "Failed to create tree context: %s\n", d_errstr(rc));
	   ...
	   uint32_t intent, d_iov_t *key, d_iov_t *val)
{
	...
	switch (rc) {
	default:
		D_ASSERTF(false, "unknown returned value: %s\n", d_errstr(rc));
		break;
...
btr_tree_alloc(struct btr_context *tcx)
{
	...
	D_DEBUG(DB_TRACE, "Allocate tree root: %s\n", d_errstr(rc));
	       ...
	       dbtree_iterate_cb_t cb, void *arg)
{
	...
	if (rc != 0) {
		D_ERROR("failed to prepare tree iterator: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
	...
	if (rc == -DER_NONEXIST) {
		...
		D_ERROR("failed to initialize iterator: %s\n", d_errstr(rc));
		D_GOTO(out_iter, rc);
	}
	...
	for (;;) {
		...
		if (rc == -DER_NONEXIST) {
			...
			D_ERROR("failed to move iterator: %s\n", d_errstr(rc));
			break;
		}
	}
...
out:
	D_DEBUG(DB_TRACE, "iterated %d records: %s\n", niterated, d_errstr(rc));
	return rc;
}
