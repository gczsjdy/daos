...
debug_fini_locked(void)
{
	...
	if (rc != 0) /* Just print a message but no need to fail */
		D_PRINT_ERR("Failed to deallocate daos debug bits: %s\n", d_errstr(rc));
...
daos_debug_init(char *logfile)
{
	...
	rc = d_log_init_adv("DAOS", logfile,
			    ...
			    DLOG_INFO, DLOG_CRIT);
	if (rc != 0) {
		D_PRINT_ERR("Failed to init DAOS debug log: %s\n", d_errstr(rc));
		goto failed_unlock;
	}

	rc = D_LOG_REGISTER_FAC(DAOS_FOREACH_LOG_FAC);
	if (rc != 0) /* Just print a message but no need to fail */
		D_PRINT_ERR("Failed to register daos log facilities: %s\n", d_errstr(rc));

	rc = D_LOG_REGISTER_DB(DAOS_FOREACH_DB);
	if (rc != 0) /* Just print a message but no need to fail */
		D_PRINT_ERR("Failed to register daos debug bits: %s\n", d_errstr(rc));

	/* Register DAOS debug bit groups */
	rc = d_log_dbg_grp_alloc(DB_GRP1, "daos_default", D_LOG_SET_AS_DEFAULT);
	if (rc < 0) {
		D_PRINT_ERR("Error allocating daos debug group: %s\n", d_errstr(rc));
		rc = -DER_UNINIT;
		goto failed_unlock;
	}
	...
	rc = daos_fail_init();
	if (rc) {
		D_PRINT_ERR("Failed to init DAOS fail injection: %s\n", d_errstr(rc));
		goto failed_unlock;
	}
