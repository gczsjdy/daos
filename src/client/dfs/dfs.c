	     ...
	     struct dfs_entry entry)
{
	...
	if (rc) {
		D_ERROR("Failed to insert entry %s (%s)\n", name, d_errstr(rc));
		return daos_der2errno(rc);
	}
	  ...
	  daos_oclass_id_t cid, daos_size_t chunk_size, dfs_obj_t *file)
{
	...
	if (rc) {
		D_ERROR("fetch_entry %s failed %s.\n", file->name,
			d_errstr(rc));
		return rc;
	}
	...
	if (flags & O_CREAT) {
		...
		if (rc != 0) {
			...
			D_ERROR("Inserting file entry %s failed (%s)\n",
				file->name, d_errstr(rc));
			return rc;
		}
	 ...
	 daos_oclass_id_t cid, dfs_obj_t *dir)
{
	...
	if (flags & O_CREAT) {
		...
		if (rc != 0) {
			...
			D_ERROR("Inserting dir entry %s failed (%s)\n",
				dir->name, d_errstr(rc));
		}
	     ...
	     const char *value, dfs_obj_t *sym)
{
	...
	if (flags & O_CREAT) {
		...
		if (rc) {
			D_FREE(sym->value);
			D_ERROR("Inserting entry %s failed (rc = %s)\n",
				sym->name, d_errstr(rc));
		}
		return rc;
	}
	return ENOTSUP;
...
open_sb(daos_handle_t coh, bool create, dfs_attr_t *attr, daos_handle_t *oh)
{
	...
	rc = daos_obj_fetch(*oh, DAOS_TX_NONE, &dkey, SB_AKEYS, iods, sgls,
			    NULL, NULL);
	if (rc) {
		D_ERROR("Failed to fetch SB info (%s)\n", d_errstr(rc));
		D_GOTO(err, rc = daos_der2errno(rc));
	}
		...
		daos_handle_t *_coh, dfs_t **_dfs)
{
	...
	daos_prop_free(prop);
	if (rc) {
		D_ERROR("daos_cont_create() failed (%s)\n", d_errstr(rc));
		return daos_der2errno(rc);
	}

	rc = daos_cont_open(poh, co_uuid, DAOS_COO_RW, &coh, &co_info, NULL);
	if (rc) {
		D_ERROR("daos_cont_open() failed (%s)\n", d_errstr(rc));
	...
	if (rc) {
		D_ERROR("Failed to insert root entry (%s).", d_errstr(rc));
		D_GOTO(err_super, rc);
	}
...
dfs_mount(daos_handle_t poh, daos_handle_t coh, int flags, dfs_t **_dfs)
{
	...
	if (rc) {
		D_ERROR("daos_cont_query() Failed (%s)\n", d_errstr(rc));
		daos_prop_free(prop);
		return daos_der2errno(rc);
	}
	...
	if (rc) {
		D_ERROR("daos_pool_query() Failed (%s)\n", d_errstr(rc));
		daos_prop_free(prop);
...
read_cb(tse_task_t *task, void *data)
{
	...
	if (rc != 0) {
		D_ERROR("Failed to get array size (%s)\n", d_errstr(rc));
		return rc;
	}
	  ...
	  daos_event_t *ev)
{
	...
	if (rc)
		D_ERROR("daos_array_write() failed (%s)\n", d_errstr(rc));
...
dfs_chmod(dfs_t *dfs, dfs_obj_t *parent, const char *name, mode_t mode)
{
	...
	if (rc) {
		D_ERROR("Failed to update mode (rc = %s)\n", d_errstr(rc));
		D_GOTO(out, rc = daos_der2errno(rc));
	}
...
dfs_osetattr(dfs_t *dfs, dfs_obj_t *obj, struct stat *stbuf, int flags)
{
	...
	rc = daos_obj_update(oh, th, &dkey, akeys_nr, iods, sgls, NULL);
	if (rc) {
		D_ERROR("Failed to update attr (rc = %s)\n", d_errstr(rc));
		D_GOTO(out_obj, rc = daos_der2errno(rc));
	}
...
dfs_punch(dfs_t *dfs, dfs_obj_t *obj, daos_off_t offset, daos_size_t len)
{
	...
	if (rc) {
		D_ERROR("daos_array_punch() failed (%s)\n", d_errstr(rc));
		return daos_der2errno(rc);
	}
	 ...
	 char *new_name, daos_obj_id_t *oid)
{
	...
	if (rc) {
		D_ERROR("Punch entry %s failed (%s)\n", name, d_errstr(rc));
		D_GOTO(out, rc = daos_der2errno(rc));
	}
	     ...
	     daos_size_t *size)
{
	...
	if (rc) {
		D_ERROR("Failed to fetch xattr %s (%s)\n", name, d_errstr(rc));
...
dfs_mount_root_cont(daos_handle_t poh, dfs_t **dfs)
{
	...
	if (rc == -DER_NONEXIST) {
		...
		if (rc)
			D_ERROR("dfs_cont_create failed (%s)\n", d_errstr(rc));
		return rc;
	}
