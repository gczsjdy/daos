...
out:
	if (rc) {
		D_ERROR("jump_map_obj_layout_fill failed, rc %s\n", d_errstr(rc));
		   ...
		   struct pl_obj_layout **layout_pp)
{
	...
	if (rc) {
		D_ERROR("jm_obj_placement_get failed, rc %s\n", d_errstr(rc));
		return rc;
	}
	...
	rc = pl_obj_layout_alloc(jmop.jmop_grp_nr * jmop.jmop_grp_size,
				 &layout);
	if (rc != 0) {
		D_ERROR("pl_obj_layout_alloc failed, rc %s\n", d_errstr(rc));
		return rc;
	}
	...
	if (rc < 0) {
		D_ERROR("Could not generate placement layout, rc %s\n", d_errstr(rc));
