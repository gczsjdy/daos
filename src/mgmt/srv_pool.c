...
ds_mgmt_tgt_pool_destroy(uuid_t pool_uuid)
{
	...
	rc = td_out->td_rc;
	if (rc != 0)
		D_ERROR(DF_UUID": failed to update pool map on %s targets\n",
...
pool_rec_delete(struct mgmt_svc *svc, uuid_t uuid)
{
	...
	if (rc != 0) {
		D_ERROR("failed to delete pool "DF_UUID" from directory: %s\n",
		    ...
		    daos_prop_t *prop, uint32_t svc_nr, d_rank_list_t **svcp)
{
	...
	*svcp = d_rank_list_alloc(svc_nr);
	...
	if (rc) {
		D_ERROR("create pool "DF_UUID" svc failed: rc %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
		goto out_svcp;
	}
	...
	if (rc != 0) {
		D_ERROR("failed to mark pool "DF_UUID" ready: %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
...
out:
	D_DEBUG(DB_MGMT, "create pool "DF_UUID": %s\n", DP_UUID(pool_uuid),
		d_errstr(rc));
	return rc;
}
...
ds_mgmt_destroy_pool(uuid_t pool_uuid, const char *group, uint32_t force)
{
	...
	if (rc != 0) {
		D_ERROR("Failed to destroy pool service "DF_UUID": %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
		goto out_svc;
	}
	...
	if (rc != 0) {
		D_ERROR("Destroying pool "DF_UUID" failed, rc: %s.\n",
			DP_UUID(pool_uuid), d_errstr(rc));
		goto out_svc;
	}
...
ds_mgmt_hdlr_pool_destroy(crt_rpc_t *rpc_req)
{
	...
	rc = crt_reply_send(rpc_req);
	if (rc != 0)
		D_ERROR("crt_reply_send failed, rc: %s\n", d_errstr(rc));
}
