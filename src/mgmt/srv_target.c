	   ...
	   daos_size_t nvme_size, char *path)
{
	...
	if (rc < 0 && errno != EEXIST) {
		D_ERROR("failed to created pool directory: %s\n", d_errstr(rc));
	...
	if (rc) {
		D_ERROR("ds_pool_create failed, rc: %s\n", d_errstr(rc));
		D_GOTO(out_tree, rc);
	}
	...
	if (rc < 0) {
		D_ERROR("failed to rename pool directory: %s\n", d_errstr(rc));
...
tgt_profile_task(void *arg)
{
	...
	D_DEBUG(DB_MGMT, "profile task: rc %s\n", d_errstr(rc));
	return rc;
}
...
ds_mgmt_tgt_map_update_pre_forward(crt_rpc_t *rpc, void *arg)
{
	...
	if (rc != 0) {
		D_DEBUG(DB_MGMT, "self rank unknown: %s\n", d_errstr(rc));
		return rc;
	}

	rc = crt_group_ranks_get(group, &ranks);
	if (rc != 0) {
		D_ERROR("failed to get existing ranks: %s\n", d_errstr(rc));
		return rc;
	}
