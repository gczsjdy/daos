...
dc_mgmt_svc_rip(tse_task_t *task)
{
	...
	if (rc != 0) {
		D_ERROR("failed to attach to grp %s, rc %s.\n", args->grp,
			d_errstr(rc));
		rc = DER_INVAL;
		goto out_task;
	}
	...
	if (rc != 0) {
		D_ERROR("crt_req_create(MGMT_SVC_RIP) failed, rc: %s.\n",
			d_errstr(rc));
		D_GOTO(err_grp, rc);
	}
...
dc_mgmt_profile(uint64_t modules, char *path, bool start)
{
	...
	if (rc != 0) {
		D_ERROR("failed to attach to grp rc %s\n", d_errstr(rc));
		return -DER_INVAL;
	}
	...
	if (rc != 0) {
		D_ERROR("crt_req_create failed, rc: %s\n", d_errstr(rc));
		D_GOTO(err_grp, rc);
	}
...
err_grp:
	D_DEBUG(DB_MGMT, "mgmt profile: rc %s\n", d_errstr(rc));
...
dc_mgmt_add_mark(const char *mark)
{
	...
	rc = daos_rpc_send_wait(rpc);
err_grp:
	D_DEBUG(DB_MGMT, "mgmt mark: rc %s\n", d_errstr(rc));
	dc_mgmt_sys_detach(sys);
	return rc;
...
get_attach_info(const char *name, int *npsrs, struct dc_mgmt_psr **psrs)
{
	...
	if (rc != 0) {
		D_ERROR("GetAttachInfo call failed: %s\n", d_errstr(rc));
		goto out_dreq;
	}
	     ...
	     struct dc_mgmt_psr *psrs, crt_group_t **groupp)
{
	...
	if (rc != 0) {
		D_ERROR("failed to create group %s: %s\n", name, d_errstr(rc));
		goto err;
	}
...
dc_mgmt_init()
{
	...
	if (rc != 0)
		D_ERROR("failed to register mgmt RPCs: %s\n", d_errstr(rc));
