...
akey_fetch(struct vos_io_context *ioc, daos_handle_t ak_toh)
{
	...
	for (i = 0; i < iod->iod_nr; i++) {
		...
		if (rc != 0) {
			D_DEBUG(DB_IO, "Failed to fetch index %d: %s\n", i,
				d_errstr(rc));
			goto out;
		}
		   ...
		   struct vos_io_context *ioc)
{
	...
	if (rc != 0)
		D_ERROR("Failed to update subtree: %s\n", d_errstr(rc));
...
akey_update_recx(daos_handle_t toh, uint32_t pm_ver, daos_recx_t *recx,
		 daos_csum_buf_t *iod_csum, daos_size_t rsize,
		 struct vos_io_context *ioc)
{
		   ...
		   daos_size_t size)
{
	...
	if (rc) {
		D_ERROR("Reserve SCM for SV failed. %s\n", d_errstr(rc));
		return rc;
	}
		   ...
		   enum vos_io_stream ios)
{
	...
	if (rc)
		D_ERROR("Error on %s NVMe reservations. %s\n",
			publish ? "publish" : "cancel", d_errstr(rc));
	       ...
	       daos_iod_t *iods, d_sg_list_t *sgls)
{
	...
	if (rc) {
		D_ERROR("Update "DF_UOID" failed %s\n", DP_UOID(oid),
			d_errstr(rc));
		return rc;
	}
	...
	if (rc) {
		if (rc == -DER_INPROGRESS)
			D_DEBUG(DB_TRACE, "Cannot fetch "DF_UOID" because of "
				...
				DP_UOID(oid), d_errstr(rc));
		else
			D_ERROR("Fetch "DF_UOID" failed %s\n",
				DP_UOID(oid), d_errstr(rc));
		return rc;
	}
