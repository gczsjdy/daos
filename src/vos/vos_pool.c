...
pool_link(struct vos_pool *pool, struct d_uuid *ukey, daos_handle_t *poh)
{
	...
	rc = d_uhash_link_insert(vos_pool_hhash_get(), ukey, NULL,
				 &pool->vp_hlink);
	if (rc) {
		D_ERROR("uuid hash table insert failed: %s\n", d_errstr(rc));
		D_GOTO(failed, rc);
	}
		...
		daos_size_t nvme_sz)
{
	...
	if (rc != 0) {
		D_ERROR("Error creating blob for xs:%p pool:"DF_UUID" rc:%s\n",
			xs_ctxt, DP_UUID(uuid), d_errstr(rc));
		goto close;
	}
	...
	rc = vea_format(&umem, vos_txd_get(), vea_md, VOS_BLK_SZ,
			...
			&blob_hdr, false);
	if (rc) {
		D_ERROR("Format blob error for xs:%p pool:"DF_UUID" rc:%s\n",
			xs_ctxt, DP_UUID(uuid), d_errstr(rc));
...
vos_pool_open(const char *path, uuid_t uuid, daos_handle_t *poh)
{
	...
	if (rc != 0) {
		D_ERROR("Failed to instantiate umem: %s\n", d_errstr(rc));
		D_GOTO(failed, rc);
	}
...
vos_pool_query(daos_handle_t poh, vos_pool_info_t *pinfo)
{
	...
	if (rc) {
		D_ERROR("Failed to get SCM usage. rc:%s\n", d_errstr(rc));
	...
	if (rc) {
		D_ERROR("Failed to get NVMe usage. rc:%s\n", d_errstr(rc));
		return rc;
	}
