...
ts_close_destroy(void **state)
{
	...
	if (rc != 0) {
		D_PRINT("Tree %s failed: %s\n",
			destroy ? "destroy" : "close", d_errstr(rc));
		fail();
	}
}
...
ts_add_rect(void **state)
{
	...
	if (rc != 0) {
		D_PRINT("Parsing tree failure %s\n", d_errstr(rc));
		fail();
	}
...
start:
	...
	if (rc != 0) {
		D_PRINT("Failed to prepare iterator: %s\n", d_errstr(rc));
		fail();
	}
...
ts_cmd_run(char opc, char *args)
{
	...
	if (rc != 0)
		D_PRINT("opc=%d failed with rc=%s\n", opc, d_errstr(rc));
