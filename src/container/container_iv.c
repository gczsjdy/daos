		 ...
		 struct ds_iv_entry *entry)
{
	...
	if (rc != 0) {
		D_ERROR("failed to create tree: %s\n", d_errstr(rc));
		return rc;
	}
...
again:
	...
	if (rc < 0) {
		...
		D_DEBUG(DB_MGMT, "lookup cont: rc %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
...
cont_iv_capability_invalidate(void *ns, uuid_t cont_hdl_uuid)
{
	...
	if (rc)
		D_ERROR("iv invalidate failed %s\n", d_errstr(rc));
...
cont_iv_prop_fetch_ult(void *data)
{
	...
	if (rc) {
		D_ERROR("cont_iv_fetch failed %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
	...
	if (rc) {
		D_ERROR("cont_iv_prop_g2l failed %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}

	rc = daos_prop_copy(prop, prop_fetch);
	if (rc) {
		D_ERROR("daos_prop_copy failed %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
