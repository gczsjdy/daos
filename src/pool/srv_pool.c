...
rechoose:
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to create POOL_CREATE RPC: %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
	...
	rc = out->pro_op.po_rc;
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to create pool: %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
		D_GOTO(out_rpc, rc);
	}

	rc = daos_rank_list_copy(svc_addrs, ranks);
	D_ASSERTF(rc == 0, "daos_rank_list_copy: %s\n", d_errstr(rc));
out_rpc:
	crt_req_decref(rpc);
out_client:
...
ds_pool_svc_destroy(const uuid_t pool_uuid)
{
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to destroy pool service: %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
		return rc;
	}
		      ...
		      uint32_t map_version)
{
	...
	pool_map_decref(map);
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to create pool group: %s\n",
			 DP_UUID(svc->ps_uuid), d_errstr(rc));
		return rc;
	}
...
pool_svc_alloc_cb(d_iov_t *id, struct ds_rsvc **rsvc)
{
	...
	if (rc != ABT_SUCCESS) {
		D_ERROR("failed to create ps_lock: %s\n", d_errstr(rc));
	      ...
	      uint32_t map_version)
{
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to get ds_pool: %s\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
		return rc;
	}
...
pool_svc_step_up_cb(struct ds_rsvc *rsvc)
{
	...
	if (rc != 0) {
		if (rc == -DER_NONEXIST) {
			...
			D_ERROR(DF_UUID": failed to read pool map buffer: %s\n",
				DP_UUID(svc->ps_uuid), d_errstr(rc));
		}
		goto out_lock;
	}
	...
	if (rc != 0)
		D_ERROR(DF_UUID": failed to get pool service replica: %s\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
out_lock:
...
pool_svc_start_all(void *arg)
{
	...
	if (rc != 0)
		D_ERROR("failed to scan all pool services: %s\n", d_errstr(rc));
}
...
ds_pool_create_handler(crt_rpc_t *rpc)
{
	...
	if (rc != -DER_NONEXIST) {
		if (rc == 0)
			...
			D_ERROR(DF_UUID": failed to look up pool map: %s\n",
				DP_UUID(svc->ps_uuid), d_errstr(rc));
		D_GOTO(out_tx, rc);
	}
...
out:
	out->pro_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pri_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
}
		   ...
		   crt_bulk_t map_buf_bulk)
{
	...
	rc = out->tco_rc;
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to connect to %s targets\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
		rc = -DER_IO;
	} else {
		if (ps != NULL)
...
out_rpc:
	crt_req_decref(rpc);
out:
	D_DEBUG(DF_DSMS, DF_UUID": bcasted: %s\n", DP_UUID(svc->ps_uuid),
		d_errstr(rc));
	return rc;
}
...
ds_pool_connect_handler(crt_rpc_t *rpc)
{
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": cannot get access data for pool, rc=%s\n",
			DP_UUID(in->pci_op.pi_uuid), d_errstr(rc));
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to read pool map: %s\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
	...
	rc = pool_connect_bcast(rpc->cr_ctx, svc, in->pci_op.pi_hdl,
		...
		CRT_BULK_NULL);
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to connect to targets: %s\n",
			DP_UUID(in->pci_op.pi_uuid), d_errstr(rc));
		D_GOTO(out_pool_prop, rc);
	}
	...
	if (rc) {
		D_ERROR("pool_map_update failed %s\n", d_errstr(rc));
		D_GOTO(out_pool_prop, rc);
	}
	...
	if (rc)
		D_ERROR("pool_iv_prop_update failed %s\n", d_errstr(rc));
...
out:
	out->pco_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pci_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
}
		      ...
		      uuid_t *pool_hdls, int n_pool_hdls)
{
	...
	rc = out->tdo_rc;
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to disconnect from %s targets\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
		rc = -DER_IO;
	}
...
out:
	D_DEBUG(DF_DSMS, DF_UUID": leaving: %s\n", DP_UUID(svc->ps_uuid),
		d_errstr(rc));
	return rc;
}
...
out:
	pdo->pdo_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(pdi->pdi_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
}
		       ...
		       struct daos_pool_space *ps)
{
	...
	rc = out->tqo_rc;
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to query from %s targets\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
		rc = -DER_IO;
	} else {
		D_ASSERT(ps != NULL);
...
out_rpc:
	crt_req_decref(rpc);
out:
	D_DEBUG(DB_MD, DF_UUID": bcasted: %s\n", DP_UUID(svc->ps_uuid),
		d_errstr(rc));
	return rc;
}
...
ds_pool_query_handler(crt_rpc_t *rpc)
{
	...
	if (DAOS_FAIL_CHECK(DAOS_FORCE_PROP_VERIFY)) {
		...
		if (rc) {
			D_ERROR("pool_iv_prop_fetch failed %s\n", d_errstr(rc));
		...
		daos_prop_free(iv_prop);
		if (rc) {
			D_ERROR("iv_prop verify failed %s\n", d_errstr(rc));
			D_GOTO(out_map_version, rc);
		}
	}
...
out:
	out->pqo_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pqi_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
	daos_prop_free(prop);
}
...
out:
	out->pgo_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pgi_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
	daos_prop_free(prop);
}
...
rechoose:
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to create pool get ACL rpc: %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
		D_GOTO(out_client, rc);
	}
	...
	rc = out->pgo_op.po_rc;
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to get ACL for pool: %s\n",
			DP_UUID(pool_uuid), d_errstr(rc));
		D_GOTO(out_rpc, rc);
	}
...
replace_failed_replicas(struct pool_svc *svc, struct pool_map *map)
{
	...
	rc = ds_pool_check_failed_replicas(map, replicas, &failed_ranks,
					   &replace_ranks);
	if (rc != 0) {
		D_DEBUG(DB_MD, DF_UUID": cannot replace failed replicas: %s\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
		D_GOTO(out, rc);
	}
			...
			d_rank_list_t **replicasp)
{
	...
	rc = rdb_tx_commit(&tx);
	if (rc != 0) {
		D_DEBUG(DB_MD, DF_UUID": failed to commit: %s\n",
			DP_UUID(svc->ps_uuid), d_errstr(rc));
		D_GOTO(out_replicas, rc);
	}
...
out:
	out->pto_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pti_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
...
out:
	out->pvo_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pvi_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
}
...
ds_pool_svc_stop_handler(crt_rpc_t *rpc)
{
	...
	out->pso_op.po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->psi_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
}
...
out:
	out->po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pasi_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
}
...
out:
	out->po_rc = rc;
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pagi_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
...
out:
	...
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %s\n",
		DP_UUID(in->pali_op.pi_uuid), rpc, d_errstr(rc));
	crt_reply_send(rpc);
}
