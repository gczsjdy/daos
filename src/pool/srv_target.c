	       ...
	       struct daos_llink **link)
{
	...
	if (rc != 0) {
		D_ERROR(DF_UUID": failed to add ES pool caches: %s\n",
			DP_UUID(key), d_errstr(rc));
		goto err_iv_lock;
	}
...
pool_free_ref(struct daos_llink *llink)
{
	...
	if (rc == -DER_CANCELED)
		...
		D_ERROR(DF_UUID": failed to delete ES pool caches: %s\n",
			DP_UUID(pool->sp_uuid), d_errstr(rc));
		      ...
		      struct ds_pool **pool)
{
	...
	if (rc != 0) {
		if (arg == NULL && rc == -DER_NONEXIST)
			...
			D_ERROR(DF_UUID": failed to lookup%s pool: %s\n",
...
pool_query_one(void *vin)
{
	...
	if (rc != 0) {
		D_ERROR("Failed to query pool "DF_UUID", tgt_id: %d, rc: %s\n",
			DP_UUID(pool->sp_uuid), tid, d_errstr(rc));
		goto out;
	}
...
pool_tgt_query(struct ds_pool *pool, struct daos_pool_space *ps)
{
	...
	if (rc) {
		D_ERROR("Pool query on pool "DF_UUID" failed, rc:%s\n",
			DP_UUID(pool->sp_uuid), d_errstr(rc));
		return rc;
	}
...
ds_pool_tgt_connect_handler(crt_rpc_t *rpc)
{
	...
	rc = ds_pool_iv_ns_update(pool, in->tci_master_rank, &in->tci_iv_ctxt,
				  in->tci_iv_ns_id);
	if (rc) {
		D_ERROR("attach iv ns failed rc %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
...
out:
	out->tco_rc = (rc == 0 ? 0 : 1);
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %d (%s)\n",
		DP_UUID(in->tci_uuid), rpc, out->tco_rc, d_errstr(rc));
	crt_reply_send(rpc);
}
...
out:
	out->tdo_rc = (rc == 0 ? 0 : 1);
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %d (%s)\n",
		DP_UUID(in->tdi_uuid), rpc, out->tdo_rc, d_errstr(rc));
	crt_reply_send(rpc);
}
		       ...
		       unsigned int map_version)
{
	...
	if (buf != NULL) {
		...
		if (rc != 0) {
			D_ERROR(DF_UUID" failed to create pool map: %s\n",
				DP_UUID(pool->sp_uuid), d_errstr(rc));
			D_GOTO(out, rc);
		}
	}
	...
	if (pool->sp_map_version < map_version ||
		...
		if (map != NULL) {
			...
			if (rc != 0) {
				...
				D_ERROR(DF_UUID": failed update pl_map: %s\n",
					DP_UUID(pool->sp_uuid), d_errstr(rc));
				D_GOTO(out, rc);
			}
		...
		if (rc != 0) {
			ABT_rwlock_unlock(pool->sp_lock);
			D_ERROR(DF_UUID": failed to update pl_map: %s\n",
				DP_UUID(pool->sp_uuid), d_errstr(rc));
			D_GOTO(out, rc);
		}
...
out:
	out->tuo_rc = (rc == 0 ? 0 : 1);
	D_DEBUG(DF_DSMS, DF_UUID": replying rpc %p: %d (%s)\n",
		DP_UUID(in->tui_uuid), rpc, out->tuo_rc, d_errstr(rc));
	crt_reply_send(rpc);
}
...
ds_pool_cont_iter(daos_handle_t ph, pool_iter_cb_t callback, void *arg)
{
	...
	if (rc != 0) {
		D_ERROR("prepare co iterator failed %s\n", d_errstr(rc));
		return rc;
	}
...
pool_iter_cb(daos_handle_t ph, uuid_t co_uuid, void *data)
{
	...
	case DAOS_INTENT_REBUILD: {
		...
		rc = dss_ult_create_execute(dtx_resync_ult, &args, NULL, NULL,
					    ...
					    0);
		if (rc != 0) {
			D_ERROR("dtx_resync_ult failed, rc %s\n", d_errstr(rc));
			return rc;
		}
