...
dc_pool_init(void)
{
	...
	if (rc != 0)
		D_ERROR("failed to register pool RPCs: %s\n", d_errstr(rc));
		...
		unsigned int map_version, bool connect)
{
	...
	if (rc != 0) {
		D_ERROR("Failed to refresh placement map: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
		    ...
		    bool connect)
{
	...
	if (rc != 0) {
		D_ERROR("failed to create local pool map: %s\n", d_errstr(rc));
		return rc;
	}
...
pool_connect_cp(tse_task_t *task, void *data)
{
	...
	if (rc == -DER_TRUNC) {
		...
		if (rc == -DER_NOTREPLICA)
			rc = -DER_NONEXIST;
		D_ERROR("failed to connect to pool: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
...
pool_disconnect_cp(tse_task_t *task, void *data)
{
	...
	rc = pdo->pdo_op.po_rc;
	if (rc) {
		D_ERROR("failed to disconnect from pool: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
...
out:
	if (rc != 0)
		D_ERROR("dc_pool_l2g failed, rc: %s\n", d_errstr(rc));
	return rc;
}
...
out:
	if (rc != 0)
		D_ERROR("dc_pool_g2l failed, rc: %s\n", d_errstr(rc));
	if (pool != NULL)
		dc_pool_put(pool);
	return rc;
}
...
pool_tgt_update_cp(tse_task_t *task, void *data)
{
	...
	rc = out->pto_op.po_rc;
	if (rc != 0) {
		D_ERROR("failed to exclude targets: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
...
pool_query_cb(tse_task_t *task, void *data)
{
	...
	if (rc) {
		D_ERROR("RPC error while querying pool: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
	...
	if (rc == -DER_TRUNC) {
		...
		D_ERROR("failed to query pool: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
...
pool_evict_cp(tse_task_t *task, void *data)
{
	...
	if (rc != 0) {
		if (rc == -DER_NOTREPLICA)
			rc = -DER_NONEXIST;
		D_ERROR("failed to evict pool handles: %s\n", d_errstr(rc));
		D_GOTO(out, rc);
	}
...
out:
	tse_task_complete(task, rc);
	D_DEBUG(DF_DSMC, "Failed to list pool attributes: %s\n", d_errstr(rc));
	return rc;
}
...
out:
	tse_task_complete(task, rc);
	D_DEBUG(DF_DSMC, "Failed to get pool attributes: %s\n", d_errstr(rc));
	return rc;
}
...
out:
	tse_task_complete(task, rc);
	D_DEBUG(DF_DSMC, "Failed to set pool attributes: %s\n", d_errstr(rc));
	return rc;
}
